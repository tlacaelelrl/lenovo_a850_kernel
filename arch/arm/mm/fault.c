...
#if 0
extern void dump_mem(const char *, const char *, unsigned long, unsigned long);
#endif
/*
...
//
#if 0
        dump_mem(KERN_EMERG, "[TRAP] PC dump: ",regs->ARM_pc-20,regs->ARM_pc+20);
        extern unsigned int   VFP_last_inst_dbg[4];
        extern unsigned int   VFP_last_pc_dbg[4];
        extern unsigned int   VFP_last_tid_dbg[4];

        printk("[TRA] user data abort:0x%x\n",addr);
        printk("[TRAP]LAST[0],inst:0x%x,pc:0x%x,tid:0x%x\n",VFP_last_inst_dbg[0],VFP_last_pc_dbg[0],VFP_last_tid_dbg[0]);
        printk("[TRAP]LAST[1],inst:0x%x,pc:0x%x,tid:0x%x\n",VFP_last_inst_dbg[1],VFP_last_pc_dbg[1],VFP_last_tid_dbg[1]);
        printk("[TRAP]LAST[2],inst:0x%x,pc:0x%x,tid:0x%x\n",VFP_last_inst_dbg[2],VFP_last_pc_dbg[2],VFP_last_tid_dbg[2]);
        printk("[TRAP]LAST[3],inst:0x%x,pc:0x%x,tid:0x%x\n",VFP_last_inst_dbg[3],VFP_last_pc_dbg[3],VFP_last_tid_dbg[3]);
#endif
...
//
        if (thumb_mode(regs)){
            ...
            if (is_wide_instruction(curr_instr_fixup)) {
                ...
                if (count <= 3)
                {
                    //only fixup vpush(ed2d0000), vpop(ecdb0000)
                    if ((curr_instr_fixup & 0xffff0000) == 0xed2d0000  || (curr_instr_fixup & 0xffff0000) == 0xecbd0000)
                    {
                        regs->ARM_pc -= 2; //fix up
                        count ++;
